...
function MapReplay({ rows, index }: { rows: ReplayRow[]; index: number }) {
  ...
  useEffect(() => {
    ...
    const path = (keyLat: keyof ReplayRow, keyLon: keyof ReplayRow, color: string, weight: number, dashArray?: string) => {
      const values = rows.slice(0, index + 1).map((row) => [Number(row[keyLat]), Number(row[keyLon])] as [number, number])
      if (values.length > 1) L.polyline(values, { color, weight, opacity: 0.9, dashArray }).addTo(layers)
    }
...
function App() {
  ...
  return <main className="app-shell">
    ...
    {error ? <section className="empty-state"><div className="empty-icon">+</div><p className="eyebrow">WAITING FOR PIPELINE EXPORT</p><h2>No replay data loaded</h2><p>{error}</p><code>public/data/navigation_replay.json</code><div className="schema-note">Expected: timestamp, GPS truth, raw IMU path, fused EKF path, heading, velocity, uncertainty, GNSS state, and map-match confidence.</div></section> : <>
      ...
      <section className="chart-panel panel"><div className="panel-heading chart-heading"><div><p className="eyebrow">POSITION ERROR // METERS</p><h2>Correction performance</h2></div><div className="chart-key"><span><i className="raw-dot" />Raw dead reckoning</span><span><i className="fused-dot" />EKF fused</span></div></div><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartRows}><CartesianGrid stroke="#E8EAED" strokeDasharray="2 5" vertical={false} /><XAxis dataKey="elapsed" tickFormatter={formatTime} stroke="#80868B" tickLine={false} axisLine={false} /><YAxis stroke="#80868B" tickLine={false} axisLine={false} width={42} tickFormatter={(value) => `${value}m`} /><Tooltip contentStyle={{ background: '#FFFFFF', border: '1px solid #DADCE0', borderRadius: 8, color: '#202124' }} labelFormatter={(value) => `T+${formatTime(Number(value))}`} formatter={(value: number) => [`${value.toFixed(1)} m`]} /><Line type="monotone" dataKey="rawError" stroke="#F9AB00" strokeWidth={2} dot={false} name="Raw" /><Line type="monotone" dataKey="fusedError" stroke="#34A853" strokeWidth={3} dot={false} name="EKF" /></LineChart></ResponsiveContainer></div></section>
